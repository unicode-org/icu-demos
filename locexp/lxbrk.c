...
void showExploreBreak(LXContext *lx, const char *locale, const char *b)
{
    ...
    exploreFetchNextPattern(lx, pattern, strstr(b,"EXPLORE_Break")); 
    ...
    unum_applyPattern(nf_default, FALSE, FSWF("EXPLORE_Break_defaultPattern", "#,###.###############"), -1, NULL, NULL);
    ...
    **/
    ...
    exploreShowPatternForm(lx, pattern, locale, "Break", strstr(b,"EXPLORE_Break"), value, nf_default); 
    ...
    else
    {
        ...
        u_fprintf(lx->OUT, "<FORM METHOD=GET ACTION=\"#EXPLORE_Break\">\r\n");
        u_fprintf(lx->OUT, "<INPUT NAME=_ TYPE=HIDDEN VALUE=%s>\r\n", locale);
        u_fprintf(lx->OUT, "<INPUT TYPE=HIDDEN NAME=EXPLORE_Break VALUE=\"");
