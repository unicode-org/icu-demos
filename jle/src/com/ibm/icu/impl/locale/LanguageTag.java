...
public final class LanguageTag {
    ...
    public static LanguageTag parse(String tag) {
        if (tag.length() < MINLEN) {
            throw new IllegalArgumentException("The specified tag '"
                    + tag + "' is too short");
        }
        ...
        while (true) {
            ...
            if ((next & VART) != 0) {
                if (isVariantSubtag(subtags[idx])) {
                    if (varBuf == null) {
                        varBuf = new StringBuilder(subtags[idx++]);
                    } else {
                        varBuf.append(SEP);
            ...
            throw new IllegalArgumentException("The specified tag '" + tag
